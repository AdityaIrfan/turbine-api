...
func (t *towerRepository) Create(tower *models.Tower) error {
	if err := t.db.Clauses(clause.Returning{}).Create(&tower).Error; err != nil {
...
func (t *towerRepository) Update(tower *models.Tower) error {
	if err := t.db.Clauses(clause.Returning{}).Updates(&tower).Error; err != nil {
...
func (t *towerRepository) GetByIdWithSelectedFields(id string, selectedFields string) (*models.Tower, error) {
	...
	if err := t.db.Select(selectedFields).Where("id = ?", id).First(&tower).Error; err != nil {
...
func (t *towerRepository) GetByEqualNameWithSelectedFields(name string, selectedFields string) (*models.Tower, error) {
	var tower *models.Tower

	if err := t.db.Select(selectedFields).Where("LOWER(name) = LOWER(?)", name).First(&tower).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
		log.Error().Err(errors.New("ERROR QUERY GET TOWER BY ID WITH SELECTED FIELDS : " + err.Error())).Msg("")
...
func (t *towerRepository) GetAll(search string) ([]*models.Tower, error) {
	...
	if err := t.db.Where("LOWER(name) LIKE LOWER(?)", "%"+search+"%").Find(&towers).Error; err != nil {
